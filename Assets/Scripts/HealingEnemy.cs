...
public class HealingEnemy : MonoBehaviour {
    ...
    public float rate = 1.16f;

    private float range = 2.75f;
    ...
    private void OnTriggerEnter2D(Collider2D collision) {
        if(damage == false) {
            if(collision.name == "Weapon") {
                ...
                myObj = Instantiate(hee.gameObject, transform);
            } else if(collision.tag == "Throwable") {
                //take damage
                damage = true;
                health -= 40;
                StartCoroutine(Damaged());
